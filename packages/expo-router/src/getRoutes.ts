  ...
  if (node.type === 'route') {
    ...
    if (anchor) {
      ...
      if (!anchorRoute) {
        ...
        if (groupName) {
          throw new Error(
            `Layout ${node.contextKey} has invalid anchor '${anchor}' for group '(${groupName})'. Valid options are: ${validInitialRoutes}`
          );
        } else {
          throw new Error(
            `Layout ${node.contextKey} has invalid anchor '${anchor}'. Valid options are: ${validInitialRoutes}`
          );
        }
      }
